...
const CharInfo = (props) => {
    ...
    useEffect(() => {
        updateChar();
    }, [props.charId]) /* тут мы проверяем изменился ли наша id и если изменился то мы обновляем */
    // componentDidMount() {
 ...
 */
    ...
    const updateChar = () => {
        ...
        const {charId} = props;/* 4(37) вытаскиваем charId из пропсов которые мы передали в app */
        if (!charId) {
            return;
        ...
        .then(onCharLoaded)/* записываем в состояние */
        .catch(onError);/* вызываем функцию для формирования одно персонажа по Id */
